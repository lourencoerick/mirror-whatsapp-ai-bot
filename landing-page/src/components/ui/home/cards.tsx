...
export default function StepCard({ step, title, titleSize = "text-2xl", icon, description, descriptionMargin = "mt-10" }: StepCardProps) {
    return (
        <div className="h-80 w-full md:w-90 bg-accent text-card-foreground p-6 rounded-lg shadow hover:shadow-lg transition-shadow">
            ...
            <div>
                {icon && description && <p className={`text-base ${descriptionMargin} leading-relaxed`}>{icon} {description}</p>}
            </div>
        </div>
    );
}
