...
interface EvolutionApiDetails { id: string; shared_api_url?: string; logical_token_encrypted?: string; }
...
export default function CreateInboxPage() {
    ...
    const handleChannelConfigured = useCallback((details: EvolutionApiDetails | CloudApiDetails, type: ChannelType) => {
    ...
    const handleEvolutionConnectionSuccess = useCallback(() => {
        console.log("Evolution API connection successful signal received by parent.");
    ...
    const handleFinalSubmit = useCallback(async () => {
        ...
        if (!inboxName.trim()) { setFormError("O nome da Inbox está faltando."); setCurrentStep(2); return; }
        ...
        if (selectedChannelType === 'whatsapp_evolution_api' && configuredDetails.evolution) {
            ...
            if (!isEvolutionConnected) {
                setFormError("A API Evolution não está conectada. Por favor, complete o processo de conexão.");
                setCurrentStep(3);
                return;
            }
             isValidPayload = true;
        ...
        if (!isValidPayload) {
            setFormError("Os detalhes de configuração para o canal selecionado são inválidos ou estão faltando.");
            setCurrentStep(3);
            return;
        }
        ...
        try {
            ...
            const data: CreateInboxResponse | { detail: string } = await response.json();

            if (!response.ok) {
                const errorMessage = (data as { detail: string }).detail || "Falha ao criar a Inbox.";
        ...
        switch (currentStep) {
                    ...
                    <Card className="w-full max-w-2xl">
                        ...
                        <CardFooter className="flex justify-between">
                            ...
                            <Button
                                ...
                                disabled={isLoading || !inboxName.trim() || inboxName.trim().length > 100}
                            >
                                Próximo
                            </Button>
                        </CardFooter>
            ...
            case 3: // Configure Channel
                if (!selectedChannelType || !inboxName.trim()) {
                    return <p className="text-destructive">Faltando seleção de canal ou nome da inbox. Por favor, volte.</p>;
                }
                return (
                    <Card className="w-full max-w-2xl">
                        <CardHeader>
                        ...
                        <CardContent>
                            {selectedChannelType === 'whatsapp_evolution_api' && (
                                <ConfigureEvolutionApiStep
                                    inboxName={inboxName}
                                    onConfigured={(details) => handleChannelConfigured(details, 'whatsapp_evolution_api')}
                            ...
                            {selectedChannelType === 'whatsapp_cloud_api' && (
                                <ConfigureCloudApiStep
                                    onConfigured={(details) => handleChannelConfigured(details, 'whatsapp_cloud_api')}
                                    onValidityChange={handleConfigValidityChange}
                                    isLoading={isLoading}
                                />
                            )}
                         ...
                         <CardContent className="space-y-4">
                             ...
                             <div className="space-y-1 rounded-md border bg-muted/50 p-4 text-sm">
                                ...
                                {selectedChannelType === 'whatsapp_evolution_api' && configuredDetails.evolution && (
                                    <>
                                        <p><strong>ID da Instância:</strong> {configuredDetails.evolution.id}</p>
                                        {/* <p><strong>URL da API:</strong> {configuredDetails.evolution.api_url}</p> */}
                                        <p><strong>Status da Conexão:</strong> <span className={isEvolutionConnected ? 'text-green-600' : 'text-destructive'}>{isEvolutionConnected ? 'Conectado' : 'Não Conectado'}</span></p> {/* PT-BR Status Text */}
                                    </>
                                )}
                                {selectedChannelType === 'whatsapp_cloud_api' && configuredDetails.cloud && (
                                    <>
                                        <p><strong>ID do Número de Telefone:</strong> {configuredDetails.cloud.phoneNumberId}</p>
                                        <p><strong>WABA ID:</strong> {configuredDetails.cloud.wabaId}</p>
