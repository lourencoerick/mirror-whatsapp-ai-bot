...
export type InboxCreatePayload = components["schemas"]["InboxCreate"];
export type InboxUpdatePayload = components["schemas"]["InboxUpdate"];
...
export const deleteInbox = async (
  ...
  if (response.ok && response.status !== 204) {
    console.warn(
      `Delete request for inbox ${inboxId} returned status ${response.status} instead of 204, but was 'ok'.`
    );
    return;
  }
};
