...
export const offeringValidationSchema = z.object({
  ...
  link: z.string().url({ message: 'Please enter a valid URL.' }).or(z.literal('')).nullable().optional(),
});
...
export const companyProfileValidationSchema = z.object({
  ...
  website: z.string().url({ message: 'Please enter a valid URL.' }).or(z.literal('')).nullable().optional(), // Permite URL, string vazia, null ou undefined
  address: z.string().max(500).nullable().optional(),
  business_description: z.string().min(1, { message: 'Business description is required.' }).max(5000), // Max length maior
  target_audience: z.string().max(500).nullable().optional(),
  sales_tone: z.string().min(1, { message: 'Sales tone is required.' }).max(255),
  language: z.string().min(1, { message: 'Language is required.' }).max(10), // Ex: 'pt-BR'
  communication_guidelines: z.array(z.string()).optional(),
  ai_objective: z.string().min(1, { message: 'AI objective is required.' }).max(1000),
...
export type CompanyProfileFormData = z.infer<typeof companyProfileValidationSchema>;
