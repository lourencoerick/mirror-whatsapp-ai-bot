...
export function InboxList() {
  ...
  const fetchAndSetInboxes = useCallback(
   ...
   */
  const handleConfigureInbox = useCallback(
    (inboxId: string) => {
  ...
  const handleOpenDeleteConfirm = useCallback(
    (inboxId: string) => {
      ...
      if (inboxToDelete) {
        ...
        console.warn(
          "Attempted to delete an inbox not found in the current list:",
          inboxId
        );
        toast.error(
          "Não foi possível encontrar a caixa de entrada para excluir."
        );
      }
    },
    [inboxes]
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    const toastId = toast.loading(
      `Excluindo a caixa de entrada "${deleteConfirm.inboxName}"...`
    );

    try {
      await inboxService.deleteInbox(deleteConfirm.inboxId, authenticatedFetch);
      toast.success(
        `Caixa de entrada "${deleteConfirm.inboxName}" excluída com sucesso.`,
        { id: toastId }
      );
      setDeleteConfirm({
        isOpen: false,
      ...
      toast.error(`Falha ao excluir a caixa de entrada: ${message}`, {
        id: toastId,
      });
        ...
        {isLoading ? (
          ...
          <Alert variant="destructive">
            ...
            <AlertDescription>
              ...
              <Button
                variant="link"
                size="sm"
                onClick={() => fetchAndSetInboxes(true)}
          ...
          >
            ...
            <h3 className="text-lg font-medium">
              Ainda não há caixas de entrada
            </h3>
