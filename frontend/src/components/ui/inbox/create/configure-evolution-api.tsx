...
export const ConfigureEvolutionApiStep: React.FC<ConfigureEvolutionApiStepProps> = ({
    ...
    const updateStatus = useCallback((newStatus: ConnectionStatus, errorMsg: string | null = null) => {
        setStatus(newStatus);
        setError(errorMsg);
        if (onStatusChange) {
    ...
    const handleTimeout = useCallback(() => {
        const msg = "Tempo esgotado esperando a leitura do QR code. Tente atualizar o código.";
    ...
    const fetchQrCode = useCallback(async (details: EvolutionInstanceDetails | null) => {
        if (!details?.id) {
            updateStatus('ERROR', 'ID da instância não encontrado para buscar QR code.');
            return;
        }
    ...
    const createInstance = useCallback(async () => {
        ...
        try {
            ...
            if (onConfigured) { // Call only if provided (creation flow)
                onConfigured(data);
            }
    ...
    useEffect(() => {
        ...
        if (status === 'IDLE') {
            if (existingInstanceId) {
                ...
                console.log(`[ConfigureEvolutionApiStep] Using existing instance ID: ${existingInstanceId}`);
                // Set minimal details needed to proceed (just the ID)
                const initialDetails: EvolutionInstanceDetails = { id: existingInstanceId };
        ...
        <div className="flex flex-col items-center space-y-4 text-center p-4 border rounded-lg bg-muted/30">
            ...
            {(status === 'ERROR' || status === 'TIMEOUT' || status === 'SOCKET_ERROR') && error && (
                <Alert variant="destructive" className="w-full max-w-md text-left">
                    ...
                    <AlertTitle>
                        {status === 'TIMEOUT' ? 'Tempo Esgotado' : status === 'SOCKET_ERROR' ? 'Erro de Conexão' : 'Erro na Configuração'}
                    </AlertTitle>
                    ...
                    <Button
                        variant="secondary"
                        size="sm"
                        onClick={() => instanceDetails ? fetchQrCode(instanceDetails) : createInstance()} // Retry fetch if details exist, else retry create
                        className="mt-4"
                        disabled={isProcessing}
                    >
                        <RefreshCw className="mr-2 h-4 w-4" />
                        {instanceDetails ? 'Atualizar QR Code' : 'Tentar Novamente'}
                    </Button>
                </Alert>
            )}
            ...
            {(status === 'CREATING_INSTANCE' || status === 'FETCHING_QR') && (
                <>
                    ...
                    <p className="text-muted-foreground">
                        {status === 'CREATING_INSTANCE' ? 'Criando instância...' : 'Gerando QR code...'}
                    </p>
                </>
            )}
            ...
            {status === 'WAITING_SCAN' && qrCode && (
                <>
                    ...
                    <div className="inline-block rounded-lg bg-white p-4 shadow-md">
                        <QRCodeSVG value={qrCode} size={220} level="M" /> {/* Slightly smaller size */}
                    </div>
            ...
            {status === 'CONNECTED' && (
                <div className="flex flex-col items-center gap-2 text-green-600">
                    ...
                    <p className="text-sm text-muted-foreground">A conexão foi estabelecida.</p>
                </div>
            )}
        </div>
    );
