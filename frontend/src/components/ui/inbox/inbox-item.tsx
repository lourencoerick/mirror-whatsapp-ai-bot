...
export const InboxItem: React.FC<InboxItemProps> = ({
    ...
    return (
        <div
            ...
            onKeyDown={isCardClickable ? (e) => { if (e.key === 'Enter' || e.key === ' ') handleCardClick(); } : undefined} // Keyboard activation
        >
            {/* Left Side: Name and Type */}
            <div className="flex flex-col gap-1 overflow-hidden mr-2"> {/* Added overflow-hidden and margin */}
                <h3
                ...
                <Badge variant="secondary" className="w-fit capitalize"> {/* Added capitalize */}
                    {inbox.channel_type.replace(/_/g, ' ')} {/* Make channel type more readable */}
                </Badge>
            </div>

            {/* Right Side: Actions (visible on hover or focus-within) */}
            {/* Using group-hover and group-focus-within for better accessibility */}
            <div className="flex items-center gap-1 sm:gap-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus-within:opacity-100 transition-opacity flex-shrink-0"> {/* Added flex-shrink-0 */}
                 {/* Configure Button */}
                 {onConfigureClick && (
                    <Button
                        variant="ghost"
