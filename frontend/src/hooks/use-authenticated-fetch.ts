    ...
    async (
      ...
      if (!isLoaded) {
        const warningMessage =
          "[useAuthenticatedFetch] Clerk auth state not loaded yet. Request blocked.";
      ...
      if (!isSignedIn) {
        const errorMessage =
          "[useAuthenticatedFetch] User is not signed in. Request blocked.";
      ...
      if (!token) {
        const errorMessage =
          "[useAuthenticatedFetch] Authentication token could not be retrieved from Clerk.";
        console.error(errorMessage);
        log.error(errorMessage);
      ...
      const backendApiUrl =
        process.env.NEXT_PUBLIC_API_BASE_URL || "http://localhost:8000"; // Default for local dev
      ...
      try {
        ...
        if (response.status === 401) {
          const errorDetail =
            "[useAuthenticatedFetch] Received 401 Unauthorized. Token might be expired or invalid.";
        ...
        if (response.status === 403) {
          const errorDetail =
            "[useAuthenticatedFetch] Received 403 Forbidden. User may not have permission or account not provisioned.";
          console.error(errorDetail);
          log.error(errorDetail);
