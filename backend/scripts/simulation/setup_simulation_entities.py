...
async def setup_entities(db: AsyncSession):
    ...
    try:
        ...
        if not account:
            ...
            account = Account(id=SIMULATION_ACCOUNT_ID, name=SIMULATION_ACCOUNT_NAME)
            db.add(account)
            await db.flush()
            logger.info(f"Account '{account.name}' (ID: {account.id}) ensured.")
        else:
            logger.info(f"Account '{account.name}' (ID: {account.id}) already exists.")

        logger.info(f"Creating  company profile for account {account.name}")
        ...
        profile = await profile_repo.get_or_create_profile(
            db=db,
        ...
        if not contact:
            ...
            logger.info(f"Contact '{contact.name}' (ID: {contact.id}) already exists.")
