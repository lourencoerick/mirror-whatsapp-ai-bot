...
def upgrade() -> None:
    ...
    op.alter_column('beta_testers', 'email',
               existing_type=sa.VARCHAR(),
               nullable=False)
    op.alter_column('beta_testers', 'notes_by_admin',
               existing_type=sa.TEXT(),
               comment='Internal notes by admin',
               existing_comment='Internal notes by admin regarding this applicant',
               existing_nullable=True)
    op.alter_column('beta_testers', 'approved_by_admin_id',
               existing_type=sa.VARCHAR(),
               comment='Clerk User ID of admin',
               existing_nullable=True)
    op.alter_column('beta_testers', 'user_id',
    ...
    op.create_index(op.f('ix_beta_testers_account_id'), 'beta_testers', ['account_id'], unique=True)
    op.create_index(op.f('ix_beta_testers_user_id'), 'beta_testers', ['user_id'], unique=True)
    op.drop_constraint('beta_testers_user_id_fkey', 'beta_testers', type_='foreignkey')
    op.drop_constraint('beta_testers_account_id_fkey', 'beta_testers', type_='foreignkey')
    op.create_foreign_key(None, 'beta_testers', 'accounts', ['account_id'], ['id'], ondelete='CASCADE')
    op.create_foreign_key(None, 'beta_testers', 'users', ['user_id'], ['id'], ondelete='CASCADE')
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(None, 'beta_testers', type_='foreignkey')
    op.drop_constraint(None, 'beta_testers', type_='foreignkey')
    op.create_foreign_key('beta_testers_account_id_fkey', 'beta_testers', 'accounts', ['account_id'], ['id'])
    op.create_foreign_key('beta_testers_user_id_fkey', 'beta_testers', 'users', ['user_id'], ['id'])
    ...
    op.create_unique_constraint('beta_testers_account_id_key', 'beta_testers', ['account_id'])
    op.alter_column('beta_testers', 'user_id',
               existing_type=sa.UUID(),
               nullable=True)
    op.alter_column('beta_testers', 'approved_by_admin_id',
